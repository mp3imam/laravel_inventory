...
function loadMailData(datas) {
    ...
    Array.from(datas).forEach(function (mailData, index) {
        ...
        <div class="col-mail col-mail-1">\
            ...
            <a href="javascript: void(0);" class="title"><span class="title-name">' + mailData.user_name + '</span> '  + '</a>\
            <span hidden class="title-to">' + mailData.to + '</span>\
        </div>\
        <div class="col-mail col-mail-2">\
            <a href="javascript: void(0);" class="subject"><span class="subject-title">' + mailData.satker_name + '</span> – <span class="teaser">' + mailData.layanan_name + '</span>\
            </a>\
            <a href="javascript: void(0);" class="subject" style="display: none;"><span class="subject-title judul">' + mailData.title + '</span> – <span class="teaser">' + mailData.body + '</span>\
            </a>\
            <div class="date " style="padding-left:0px;"><small>' + mailData.created_at + '<small></div>\
        </div>\
    </li>';
        favouriteBtn();
        emailDetailShow();
...
// get json
getJSON("mail", function (err, data) {
    if (err !== null) {
        console.log("Something went wrong: " + err);
    } else {
        allmaillist = data.data;
...
function emailDetailChange() {
    Array.from(document.querySelectorAll(".message-list li")).forEach(function (item) {
        item.addEventListener("click", function () {
            ...
            var subjectTitle = item.querySelector(".judul").innerHTML;
            ...
            var emailTitleLefto = item.querySelector(".title-to").innerHTML;
            var emailTitleLefBody = item.querySelector(".teaser").innerHTML;
            var emailTitleLefDate = item.querySelector(".date").innerHTML;
            ...
            Array.from(document.querySelectorAll(".accordion-item.left")).forEach(function (subitem) {
                ...
                subitem.querySelector(".email-to").innerHTML = emailTitleLefto;
                subitem.querySelector(".teaser").innerHTML = emailTitleLefBody;
                subitem.querySelector(".date").innerHTML = emailTitleLefDate;
